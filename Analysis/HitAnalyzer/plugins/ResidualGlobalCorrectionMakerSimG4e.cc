...
class ResidualGlobalCorrectionMakerSimG4e : public ResidualGlobalCorrectionMakerBase
{
public:
  explicit ResidualGlobalCorrectionMakerSimG4e(const edm::ParameterSet &);
  ~ResidualGlobalCorrectionMakerSimG4e() {}
...
ResidualGlobalCorrectionMakerSimG4e::ResidualGlobalCorrectionMakerSimG4e(const edm::ParameterSet &iConfig) : ResidualGlobalCorrectionMakerBase(iConfig) 
{
  
}

void ResidualGlobalCorrectionMakerSimG4e::beginStream(edm::StreamID streamid)
{
...
//       ;
      ...
      auto propresult = g4prop->propagateWithPath(refFts, det0->surface());
...
//         const Matrix<double, 5, 6> EdE = materialEffectsJacobian(updtsos, fPropagator->materialEffectsUpdator());
        const Matrix<double, 5, 6> EdE = Matrix<double, 5, 6>::Zero();
...
//         const std::array<Matrix<double, 5, 5>, 5> dQs = processNoiseJacobians(updtsos, fPropagator->materialEffectsUpdator());
        const std::array<Matrix<double, 5, 5>, 5> dQs;
...
//           }
          ...
          propresult = g4prop->propagateWithPath(updtsos, detplus1->surface());
          if (!propresult.first.isValid()) {
            std::cout << "Abort: Propagation Failed!" << std::endl;
            valid = false;
            break;
...
DEFINE_FWK_MODULE(ResidualGlobalCorrectionMakerSimG4e);
