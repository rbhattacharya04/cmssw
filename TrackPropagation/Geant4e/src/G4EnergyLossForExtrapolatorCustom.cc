					     ...
					     G4double kinEnergy)
{
  ...
  if(mat != currentMaterial) {
    ...
    if(i >= nmat) {
      G4cout << "### G4EnergyLossForExtrapolatorCustom WARNING:index i= " 
...
{
  if(verbose>1) {
    G4cout << "### G4EnergyLossForExtrapolatorCustom::Initialisation" << G4endl;
  }
  currentParticle = nullptr;
  currentMaterial = nullptr;
  kineticEnergy   = 0.0;
...
namespace { G4Mutex G4EnergyLossForExtrapolatorCustomMutex = G4MUTEX_INITIALIZER; }

void G4EnergyLossForExtrapolatorCustom::BuildTables()
{
  G4AutoLock l(&G4EnergyLossForExtrapolatorCustomMutex);

  if(!tables) {
    if(verbose > 0) {
      G4cout << "### G4EnergyLossForExtrapolatorCustom::BuildTables for "
