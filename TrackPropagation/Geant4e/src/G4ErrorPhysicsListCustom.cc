...
//------------------------------------------------------------------------
G4ErrorPhysicsListCustom::G4ErrorPhysicsListCustom():  G4VUserPhysicsList()
{
...
void G4ErrorPhysicsListCustom::ConstructProcess()
{
  ...
  while( (*myParticleIterator)() ) {  // Loop checking, 06.08.2015, G.Cosmo
    ...
    if (!particle->IsShortLived()) {
      G4cout << particle << "G4ErrorPhysicsListCustom:: particle process manager " << particle->GetParticleName() << " = " << particle->GetProcessManager() << G4endl;
      // Add transportation process for all particles other than  "shortlived"
      if ( pmanager == 0) {
        // Error !! no process manager
        G4String particleName = particle->GetParticleName();
        G4Exception("G4ErrorPhysicsListCustom::ConstructProcess","No process manager",
...
void G4ErrorPhysicsListCustom::ConstructEM()
{
  ...
  new G4ErrorMessengerCustom( stepLengthLimitProcess, magFieldLimitProcess, eLossProcess );
